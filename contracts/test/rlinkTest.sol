...
contract rlinkDemo is ReentrancyGuard {
    using SafeERC20 for IERC20;
    
    address public stakingToken;
    address public rlinkRelation;
    ...
    constructor(
        ...
        IERC20(_stakingToken).safeApprove(_rlinkRelation, type(uint256).max);
    }

    function isParent(address child,address parent) external view returns(bool){
        return IRlinkRelation(rlinkRelation).isParent(child, parent);
    }    
    ...
    function addRelation(address child,address parent) external nonReentrant {
        require(child == msg.sender,"child must be tx sender");
        uint oldBalance = IERC20(stakingToken).balanceOf(address(this));
        require(IRlinkRelation(rlinkRelation).addRelation(child, parent),"add relation failed");
        uint newBalance = IERC20(stakingToken).balanceOf(address(this));

        if(newBalance > oldBalance){
            uint256 rewardsAmount = newBalance - oldBalance; 
            uint256 incentiveRate = 0;
            uint256 parentRate = 15 * 1e16;
            uint256 grandpaRate = 5 * 1e16;
            require(IRlinkRelation(rlinkRelation).distribute(stakingToken, msg.sender, rewardsAmount, incentiveRate, parentRate, grandpaRate) > 0,"distribute failed");        
        }
    }
}